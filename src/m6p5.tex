...
{\Script} di atas merupakan kumpulan-kumpulan data dan struktur-struktur data yang diperlukan untuk jalan kerjanya program. Struktur data \citecode{Function} digunakan untuk menyimpan masukan fungsi dan struktur data \citecode{Stack} digunakan untuk menyimpan \textit{stack} secara keseluruhan.
...
{\Script} di atas digunakan untuk mendapatkan masukan dari pengguna ketika berada pada menu \textit{debug mode}. Fungsi \citecode{debug\_mode} digunakan untuk mengatur semua hal yang berkaitan dengan \textit{debug mode} seperti menunjukan \textit{register} dan \textit{stack}.
...
{\Script} di atas merupakan fungsi yang digunakan untuk menulusuri data-data yang tersimpan dalam \citecode{stack}. Penampilannya akan dipisah menjadi dua bagian yakni \textit{stack} dan \textit{register} yang akan berubah sesuai nilai yang terkandung pada variabel \citecode{stack}.

\begin{code}
\begin{verbatim}
int main() { int state = 0;
  while (true) {
    topbar();
    cout << "1. Add Data\n";
    cout << "2. Debug Mode\n";
...
struct Item {
  int health, attack, defense; string name; };
...
\begin{verbatim}
  ...
  switch (room_id) {
    case 1: line = 7; break;
    case 2: line = 10; break;
    case 3: line = 14; column = 24; break;
    case 4: line = 14; column = 18; break;
    case 5: line = 13; column = 34; break;
    case 6: line = 18; column = 34; break;
  }
  cout << "\e7";
...
{\Script} di atas merupakan bagian dari fungsi \citecode{game\_create}. Potongan fungsi tersebut digunakan untuk mengisi data terkait sebuah monster yang bernama Phoenix. Monster tersebut memiliki kehidupan sebanyak \citecode{950}, kekuatan serangan \citecode{140} dan pertahanan \citecode{10}.
...
{\Script} di atas merupakan bagian dari fungsi \citecode{game\_create}. Potongan fungsi tersebut digunakan untuk mengisi data terkait sebuah monster yang bernama Leviathan. Monster tersebut memiliki kehidupan sebanyak \citecode{5000}, kekuatan serangan \citecode{350} dan perhatanan \citecode{50}.
...
if (no_monster) {
  ...
  if (game.rooms[game.current_room_id].item_id > -1) {
    ...
    center_draw(item.name); item_info(item);
    bar_draw();
...
{\Script} di atas merupakan potongan dari fungsi \citecode{game\_draw} yang digunakan untuk menggambar keseluruhan permainan. Potongan fungsi tersebut digunakan untuk menggambar properti monster ketika sebuah ruangan berisi monster.
...
cout << strint(game.players[1].attack) + " : Atk"
     << "| |" << endl;
\end{verbatim}
\end{code}
...
{\Script} di atas merupakan potongan dari fungsi \citecode{game\_draw} yang digunakan untuk menggambar keseluruhan permainan. Potongan fungsi tersebut digunakan untuk menggambar aksi-aksi yang bisa diambil pemain pada saat sebuah monster terdapat pada ruangan.
...
{\Script} di atas merupakan potongan dari fungsi \citecode{game\_update} yang digunakan untuk mengatur interaksi pemain dengan permainan. Potongan fungsi tersebut akan mengatur apa yang akan terjadi bila pemain memilih untuk menyerang monster dan juga mengatur apakah \textit{skill} pemain satu terjadi.
...
{\Script} di atas merupakan potongan dari fungsi \citecode{game\_update} yang digunakan untuk mengatur interaksi pemain dengan permainan. Potongan fungsi tersebut akan mengatur apa yang akan terjadi bila pemain memilih untuk menyembuhkan diri yakni menambahkan \citecode{50} kesehatan pada pemain.
...
{\Script} di atas merupakan potongan dari fungsi \citecode{game\_update} yang digunakan untuk mengatur interaksi pemain dengan permainan. Potongan fungsi tersebut akan mengatur \textit{skill} spesial monster yang akan terjadi setiap 5 kali giliran monster.
...
{\Script} di atas merupakan potongan dari fungsi \citecode{game\_update} yang digunakan untuk mengatur interaksi pemain dengan permainan. Potongan fungsi tersebut akan mengatur serangan tanpa \textit{skill} yang dilakukan monster.
...
{\Script} di atas merupakan potongan dari fungsi \citecode{game\_update} yang digunakan untuk mengatur interaksi pemain dengan permainan. Potongan fungsi tersebut akan menambahkan nilai giliran pemain saat ini dan monster setelah semua aksi terjadi.
...
for (int i = 0; i < 2; ++i) 
  if (game->players[i].health <= 0) {
    game->state = GAME_STATE_LOSE; break; }
\end{verbatim}
\end{code}

{\Script} di atas merupakan potongan dari fungsi \citecode{game\_update} yang digunakan untuk mengatur interaksi pemain dengan permainan. Potongan fungsi tersebut digunakan untuk mengatur kondisi permainan apakah menang atau kalah tergantung pada kondisi apakah salah satu pemain mati atau monster bernama Leviathan kalah.
