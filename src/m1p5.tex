...
{\Script} \citecode{cout << "Suhu awal: "} akan memasukan teks \citecode{Suhu Awal: } ke dalam {\ioo} {\stream} dengan perantara objek \citecode{cout}. Semua teks yang berada di dalam {\ioo} {\stream} akan ditampilkan kepada pengguna dalam bentuk teks.
...
{\Script} \citecode{cin >> temp} adalah {\statement} {\ioi} yang mengambil masukan dari \textit{standard} {\ioi} {\stream} dan memasukannya ke dalam variabel \citecode{temp}. Hal ini juga yang mengakibatkan variabel \citecode{temp} tidak diinisialisasikan seperti yang telah dijelaskan sebelumnya.
...
{\Script} \citecode{cout << endl;} akan memasukan \textit{newline escape sequence} dan mem-\textit{flush} terhadap isi \textit{standard output stream}. Dalam bahasa C++ sebuah isi \textit{standard output stream} akan ditampilkan bila ditemukan \textit{newline escape sequence}, hal ini disebut dengan \textit{buffering}.
