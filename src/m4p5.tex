...
{\Script} di atas merupakan sebuah fungsi tanpa kembalian yang ditandakan oleh kata kunci \citecode{void}. Fungsi tersebut digunakan untuk mencetak {\header} atau \textit{bar} pada bagian atas program ketika fungsi tersebut dipanggil dalam program.
...
void unlogged_page() {
  switch ((app_state & 0x6) >> 0x1) {
    ...
    case 1: {
      ...
      if (app_state = login(name, password)) cout << "Log-In Berhasil\n";
...
{\Script} di atas digunakan untuk menampilkan daftar makanan yang tersimpan pada variabel {\sarray} \citecode{foods}. Jumlah makanan yang akan ditampilkan sesuai dengan nilai variabel \citecode{food\_count} yang bertipe data \citecode{int} yang bisa menyimpan bilangan bulat.
...
{\Script} di atas digunakan untuk menampilkan data penyewa berupa daftar nama-nama pengguna yang tersedia pada variabel {\sarray} \citecode{users} dan tagihan pengguna yang tersedia pada variabel {\sarray} \citecode{user\_bills} yang digunakan untuk menyimpan tagihan semua pengguna.
...
{\Script} di atas digunakan untuk membuat matriks segitiga atas dengan cara membagi bagian segitiga bawah dari \citecode{matrix} menjadi 0. Nilai yang digunakan untuk membaginya disimpan pada \citecode{x} yang akan dihitung setiap kali sebuah elemen matriks diubah jadi 0.
